#!/usr/bin/env python3
"""Train one MultiModalSCVI sweep config and cache its artifacts.

One run = one point in the grid (spatial x n_latent x n_layers x batch_mask x experts).
Idempotent: skips if <out-root>/<run_id>/DONE exists.
"""
import argparse
import json
import sys
...
def make_run_id(a):
    return f'sp-{a.spatial}__nl{a.n_latent}__L{a.n_layers}__bm-{a.batch_mask}__{a.experts}'
...
def main():
    ...
    p.add_argument('--experts', choices=EXPERTS, required=True)
    p.add_argument('--out-root', default=str(CACHE / 'sweep'))
    ...
    from PixelGen.utils import build_spatial_celltype_obsm, calculate_metrics, get_dense
    ...
    model_kwargs = dict(
        ...
        experts_method=EXPERTS[args.experts],
        loss_weights='auto',
    ...
    print(f'[train] {run_id}  spatial_key={spatial_key}  mask={BATCH_MASK[args.batch_mask]}')
